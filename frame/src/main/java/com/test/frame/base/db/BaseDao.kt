package com.test.frame.base.db
...
abstract class BaseDao<T> {
    ...
    fun deleteByParams(params: String, value: String): Int {
        ...
        Log.e("BaseDao", "deleteByParams: ${"delete from $tableName where $params='${value}'"}")
    ...
    fun doQueryByLimit(vararg string: String, limit: Int = 10, offset: Int = 0): List<T>? {
        val query =
            SimpleSQLiteQuery("SELECT * FROM $tableName WHERE ${string[0]} ${string[1]} '${string[2]}' ORDER BY ${string[0]} limit $limit offset $offset")
        return doQueryByLimit(query)
    }

    fun doQueryByLimit(vararg string: String): List<T>? {
        val s = "SELECT * FROM $tableName WHERE ${string[0]} ${string[1]} '${string[2]}'"
        Log.e(TAG,s)
        val query = SimpleSQLiteQuery(s)
        return doQueryByLimit(query)
    }
     ...
     * 降序分页查询
     */
    fun doQueryByOrder(vararg string: String): List<T> {
        val query =
            SimpleSQLiteQuery("SELECT * FROM $tableName ORDER BY ${string[0]} desc")
        return doQueryByLimit(query)
    }
    ...
    private val tableName: String
        get() {
            ...
            Log.e("BaseDao", "getTableName: -->$tableName")
            return tableName
        }
